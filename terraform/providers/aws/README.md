...
This project will deploy an end to end infrastructure in AWS that includes the below resources in `us-west-2`.
...
You do **NOT** want to queue builds for `aws-us-west-2-ubuntu-nodejs` because this Build Template will be used by the application. Queueing a build for `aws-us-west-2-ubuntu-nodejs` **will fail** with the error `* Bad source 'app/': stat app/: no such file or directory`.

#### Base Artifact

- [ ] Create `aws-us-west-2-ubuntu-base` Artifact
  - [ ] [Create New Build Configuration](https://atlas.hashicorp.com/builds/new) for `aws-us-west-2-ubuntu-base`
...
For child Build Configurations, there is one additional step you need to take. In "Settings", set **Inject artifact ID during build** to `aws-us-west-2-ubuntu-base` for each.

- [ ] Create `aws-us-west-2-ubuntu-consul` Artifact
  - [ ] [Create New Build Configuration](https://atlas.hashicorp.com/builds/new) for `aws-us-west-2-ubuntu-consul`
    - **Name**: `aws-us-west-2-ubuntu-consul`
  - [ ] In "Settings": Set **Inject artifact ID during build** to `aws-us-west-2-ubuntu-base`
  - [ ] In "Variables": [Add Environment Variables](https://github.com/hashicorp/best-practices/blob/master/terraform/providers/aws/README.md#add-environment-variables) mentioned above
  - [ ] In "Integrations": [Setup GitHub Integration](https://github.com/hashicorp/best-practices/blob/master/terraform/providers/aws/README.md#integrate-with-github) for the `best-practices` repo
    - **Packer directory**: `packer`
    - **Packer template**: `aws/ubuntu/consul.json`
- [ ] Create `aws-us-west-2-ubuntu-vault` Artifact
  - [ ] [Create New Build Configuration](https://atlas.hashicorp.com/builds/new) for `aws-us-west-2-ubuntu-vault`
    - **Name**: `aws-us-west-2-ubuntu-vault`
  - [ ] In "Settings": Set **Inject artifact ID during build** to `aws-us-west-2-ubuntu-base`
  - [ ] In "Variables": [Add Environment Variables](https://github.com/hashicorp/best-practices/blob/master/terraform/providers/aws/README.md#add-environment-variables) mentioned above
  - [ ] In "Integrations": [Setup GitHub Integration](https://github.com/hashicorp/best-practices/blob/master/terraform/providers/aws/README.md#integrate-with-github) for the `best-practices` repo
    - **Packer directory**: `packer`
    - **Packer template**: `aws/ubuntu/vault.json`
- [ ] Create `aws-us-west-2-ubuntu-haproxy` Artifact
  - [ ] [Create New Build Configuration](https://atlas.hashicorp.com/builds/new) for `aws-us-west-2-ubuntu-haproxy`
    - **Name**: `aws-us-west-2-ubuntu-haproxy`
  - [ ] In "Settings": Set **Inject artifact ID during build** to `aws-us-west-2-ubuntu-base`
  - [ ] In "Variables": [Add Environment Variables](https://github.com/hashicorp/best-practices/blob/master/terraform/providers/aws/README.md#add-environment-variables) mentioned above
  - [ ] In "Integrations": [Setup GitHub Integration](https://github.com/hashicorp/best-practices/blob/master/terraform/providers/aws/README.md#integrate-with-github) for the `best-practices` repo
    - **Packer directory**: `packer`
    - **Packer template**: `aws/ubuntu/haproxy.json`
- [ ] Create `aws-us-west-2-ubuntu-nodejs` Build Configuration
  - [ ] [Create New Build Configuration](https://atlas.hashicorp.com/builds/new) for `aws-us-west-2-ubuntu-nodejs`
    - **Name**: `aws-us-west-2-ubuntu-nodejs`
  - [ ] In "Settings": Set **Inject artifact ID during build** to `aws-us-west-2-ubuntu-base`
  - [ ] In "Variables": [Add Environment Variables](https://github.com/hashicorp/best-practices/blob/master/terraform/providers/aws/README.md#add-environment-variables) mentioned above
  - [ ] In "Integrations": [Setup GitHub Integration](https://github.com/hashicorp/best-practices/blob/master/terraform/providers/aws/README.md#integrate-with-github) for the `best-practices` repo
    - **Packer directory**: `packer`
...
- [ ] [Click **Queue build**](https://github.com/hashicorp/best-practices/blob/master/terraform/providers/aws/README.md#queue-build) in "Builds" for each of the below Build Configurations to create new artifacts for each (remember, we will not do this for `aws-us-west-2-ubuntu-nodejs`)
  - [ ] `aws-us-west-2-ubuntu-consul`
  - [ ] `aws-us-west-2-ubuntu-vault`
  - [ ] `aws-us-west-2-ubuntu-haproxy`

We built artifacts for the `us-west-2` region in this walkthrough. If you'd like to add another region, follow the [Multi-Region](https://github.com/hashicorp/best-practices/blob/master/terraform/providers/aws/README.md#multi-region) setup instructions below.
...
### Deploy a `us-west-2` Node.js Application
...
- [ ] Use the [New Application](https://atlas.hashicorp.com/applications/new) tool to create your Node.js Application
  - [ ] **Choose a name for the application**: `aws-us-west-2-nodejs`
  - [ ] **Compile Application**: checked
  - [ ] **Build Template**: `aws-us-west-2-ubuntu-nodejs`
...
### Provision the `aws-us-west-2-prod` Environment

- [ ] Use the [Import Terraform Configuration from GitHub](https://atlas.hashicorp.com/configurations/import) tool to import the `aws-us-west-2-prod` Environment from GitHub
  - [ ] **Name the environment**: `YOUR_ATLAS_ORG/aws-us-west-2-prod`
  - [ ] **GitHub repository**: `YOUR_GITHUB_USERNAME/best-practices`
  - [ ] **Path to directory of Terraform files**: `terraform`
- [ ] [`terraform push`](https://github.com/hashicorp/atlas-examples/blob/master/setup/general.md#deploy-with-terraform) your environment to Atlas to set the Terraform variables, the GitHub Ingress does not currently pull in variables
  - [ ] [Set local environment variables](https://github.com/hashicorp/best-practices/blob/master/terraform/providers/aws/README.md#set-local-environment-variables)
  ...
  - [ ] Configure & pull remote state: `terraform remote config -backend-config name=$ATLAS_USERNAME/aws-us-west-2-prod`
  - [ ] Get latest modules: `terraform get`
  - [ ] Push to Atlas: `terraform push -name $ATLAS_USERNAME/aws-us-west-2-prod -var "atlas_token=$ATLAS_TOKEN" -var "atlas_username=$ATLAS_USERNAME"`
    - The plan in Atlas **will** fail, this is okay
- [ ] Navigate to the `aws-us-west-2-prod` [environment](https://atlas.hashicorp.com/environments)
- [ ] In "Settings": check **Plan on artifact uploads** and click **Save**
- [ ] In "Variables": add the below Environment Variables with appropriate values
  - [ ] `ATLAS_USERNAME`
  - [ ] `AWS_ACCESS_KEY_ID`
...
- [ ] In "Integrations": under "GitHub Integration" click **Update GitHub settings** to pull the latest configuration from master
- [ ] In "Changes": click **Queue plan** if one has not already been queued, then **Confirm & Apply** to provision the `aws-us-west-2-prod` environment
...
- [ ] In "Variables" of the `aws-us-west-2-prod` environment: Update `vault_token` with the `root-token`
- [ ] Commit a new change (`git commit --allow-empty -m "Force a change in Atlas"`) to your [`demo-app-nodejs` repo](https://github.com/hashicorp/demo-app-nodejs), this should trigger a new "plan" in `aws-us-west-2-prod` after a new artifact is built
- [ ] In "Changes" of the the `aws-us-west-2-prod` environment: Queue a new plan and apply the changes to deploy the new application to see the Vault/Consul Template integration at work
...
If you'd like to expand outside of `us-west-2`, there are a few changes you need to make. We'll use the region `us-west-2` as an example of how to do this.
...
- Replace all instances of `us-west-2` with `us-west-2`.
...
Finally, push these new environments to `master` and follow the [same steps](https://github.com/hashicorp/best-practices/blob/master/terraform/providers/aws/README.md#provision-the-us-west-2-prod-infrastructure-with-terraform-in-atlas) you completed to deploy your environments in `us-west-2`.
